...
var ActDetail = (function() {
  ...
  function _getData(sid) {
    ...
    if (!detail || !streams) {
      for (i = 0; i < localStorage.length; i++) {
        ...
        if (!detail && k.indexOf(sidStr) !== -1 && k.toLowerCase().indexOf('detail') !== -1) {
          try { detail = JSON.parse(localStorage.getItem(k)); } catch(e) {}
        }
        if (!streams && k.indexOf(sidStr) !== -1 && k.toLowerCase().indexOf('stream') !== -1) {
          try { streams = JSON.parse(localStorage.getItem(k)); } catch(e) {}
        }
      }
    }
  ...
  // Running Power
  function _calcPower(v, grade, accel, mass) {
    var th = Math.atan(grade);
    var grav = mass * G * v * (Math.sin(th) + CR * Math.cos(th));
    var air = 0.5 * RHO * CD * FA * Math.pow(v, 3);
    var kin = mass * accel * v;
    return Math.max(0, grav + air + kin);
  }
  ...
  function _powerStream(str) {
    ...
    for (i = 1; i < n - 1; i++) {
      ...
      pw[i] = _calcPower(vel[i], gr, ac, mass);
    }
    pw[0] = pw[1];
    pw[n-1] = pw[n-2];
    return pw;
  ...
  function _normalizedPower(pw, tm) {
    if (!pw || pw.length < 30) return null;
    var rolling = [], ws = 0, wSum = 0, wC = 0, i;
    for (i = 0; i < pw.length; i++) {
      wSum += pw[i]; wC++;
      while (tm && tm[i] - tm[ws] > 30) { wSum -= pw[ws]; wC--; ws++; }
  ...
  function _buildLaps(detail, streams, pwStream) {
    var laps = [];
    var splits = detail.splits_metric;
    if (!splits || splits.length === 0) return laps;
    var hrD = (streams && streams.heartrate) ? streams.heartrate.data : null;
    var altD = (streams && streams.altitude) ? streams.altitude.data : null;
    var dstD = (streams && streams.distance) ? streams.distance.data : null;
  ...
  function render(sid) {
    ...
    if (!act) return '<div class="empty">Brak danych szczegolowych. Zsynchronizuj Strave.</div>';

    var pwStr = str ? _powerStream(str) : null;
    var gapStr = str ? _gapStream(str) : null;
    var laps = _buildLaps(act, str, pwStr);

    var distKm = (act.distance / 1000).toFixed(2);
    var dur = _fmtTime(act.moving_time);
    var avgPace = _fmtPace(act.average_speed);
    var avgHR = act.average_heartrate ? Math.round(act.average_heartrate) : '--';
    var elev = act.total_elevation_gain ? Math.round(act.total_elevation_gain) : 0;

    var avgPwr = '--', maxPwr = '--', normPwr = '--';
    if (pwStr) {
      var sum = 0; for (var pi = 0; pi < pwStr.length; pi++) sum += pwStr[pi];
      avgPwr = Math.round(sum / pwStr.length);
      maxPwr = Math.round(Math.max.apply(null, pwStr));
      var tmD = (str && str.time) ? str.time.data : null;
    ...
    var overallGAP = '--';
    if (gapStr) {
      var gs = 0; for (var gi = 0; gi < gapStr.length; gi++) gs += gapStr[gi];
      overallGAP = _fmtPace(gs / gapStr.length);
    }

    // Build HTML
    var h = '<div class="detail-summary"><div class="stat-grid">';
    h += '<div class="stat"><span class="stat-label">Dystans</span><span class="stat-value">' + distKm + ' km</span></div>';
    h += '<div class="stat"><span class="stat-label">Czas</span><span class="stat-value">' + dur + '</span></div>';
    h += '<div class="stat"><span class="stat-label">Tempo</span><span class="stat-value">' + avgPace + ' /km</span></div>';
    ...
    // Chart containers
    var hasHR = str && str.heartrate && str.heartrate.data && str.heartrate.data.length > 0;
    var hasDst = str && str.distance && str.distance.data;
    if (hasHR && hasDst)
    ...
    if (laps.length > 0) {
      var pArr = [], gArr = [];
      for (var li = 0; li < laps.length; li++) { pArr.push(laps[li].gSec); gArr.push(laps[li].gSec); }
      var mn = Math.min.apply(null, pArr);
      var mx = Math.max.apply(null, pArr);
  ...
  function drawCharts(sid) {
    ...
    var hrD = str.heartrate ? str.heartrate.data : null;
    var dstD = str.distance ? str.distance.data : null;
    var tmD = str.time ? str.time.data : null;
    var pwStr = _powerStream(str);
    var gapStr = _gapStream(str);
    var laps = _buildLaps(act, str, pwStr);
    var i, key, cv, labels;
    ...
    if (hrD && hrD.length && dstD) {
      ...
      if (cv) {
        ...
        var zones = _hrZones();
        var hrMin = Math.min.apply(null, hrD);
        var hrMax = Math.max.apply(null, hrD);
        var zonePlugin = {
          id: 'hrZoneBg' + sid,
    ...
    if (pwStr && dstD) {
      ...
      if (cv) {
        ...
        for (i = 0; i < pwStr.length; i++) {
          var s1 = Math.max(0, i-5), e1 = Math.min(pwStr.length, i+5);
          var sm = 0;
